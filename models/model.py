...
class Cell(nn.Module):
  ...
  def __init__(self, genotype, C_prev_prev, C_prev, C, reduction, reduction_prev, layer_index):
    ...
    if reduction:
      op_names, indices = zip(*genotype.reduce[layer_index])              #getting operation name and edge index of reduction cell at index(layer index)
      concat = genotype.reduce_concat
    else:
      op_names, indices = zip(*genotype.normal[layer_index])                #getting operation name and edge index of normal cell at index(layer index)
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, genotype):
    ...
    for i in range(layers):
      ...
      cell = Cell(genotype, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, layer_index)
